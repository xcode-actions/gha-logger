...
public struct GHALogger : LogHandler {
	...
	public init(metadataProvider: Logger.MetadataProvider? = LoggingSystem.metadataProvider) {
		...
		constantsByLevel[.critical]!.logPrefix = "\(SGR(.fgColorTo4BitBrightWhite, .bgColorTo4BitBrightRed, .bold).rawValue)CRITICAL\(SGR(.bold).rawValue) " /* This will be modified dynamically, but we always want the bold and [critical]. */
